    ...
    <div className="flex items-center gap-2 text-xs">
      ...
      <div className="flex -space-x-2">
        {users.slice(0, 6).map((u) => (
          <div key={u.id} title={`${u.nickname || u.id.slice(0,4)} — ${u.score}%`} className="h-6 w-6 rounded-full overflow-hidden border border-theme bg-neutral-200">
      ...
      {users.length > 6 && (
        <div className="h-6 w-6 rounded-full bg-neutral-200 border border-theme text-[10px] grid place-items-center text-neutral-700">+{users.length - 6}</div>
      )}
    </div>
  );
}
